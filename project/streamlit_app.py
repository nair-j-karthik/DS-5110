...
def load_data():
    ...
    # Find all .csv files in sales_data directory
    sales_files = glob.glob("sales_data/**/*.csv", recursive=True)
    if not sales_files:
        st.error("No sales data files found in the 'sales_data' directory.")
        return customer_demographics, holidays, locations, pd.DataFrame()

    # Concatenate all sales data files
    sales_data = pd.concat([pd.read_csv(file) for file in sales_files], ignore_index=True)
    ...
    sales_data = sales_data.merge(
        locations[["location_id", "city", "lat", "lng"]],
        left_on="location_id",
...
location_filter = st.sidebar.selectbox("Select Location", ["All"] + list(locations["city"].unique()))
holiday_filter = st.sidebar.selectbox("Select Holiday", ["All"] + list(holidays["holiday_name"].dropna().unique()))

# Debugging Information (for testing purposes)
...
if location_filter != "All":
    filtered_data = filtered_data[filtered_data["city"] == location_filter]  # Ensure this matches column name in sales_data
...
if not filtered_data.empty:
    ...
    sales_by_location = filtered_data.groupby("city")["sales_amount"].sum().nlargest(10)
    with col2:
...
if not filtered_data.empty:
    ...
    heatmap_data = filtered_data.groupby(["lat", "lng"])["sales_amount"].sum().reset_index()
    heatmap_map = folium.Map(location=[39.8283, -98.5795], zoom_start=4)  # Center of the USA
    HeatMap(data=heatmap_data[["lat", "lng", "sales_amount"]].values.tolist()).add_to(heatmap_map)
